...
class QueueGuardHandler(InterventionHandler):
    ...
    arbitrary url straight through to yt-dlp on the bot side, a real SSRF risk
    (yt-dlp's generic extractor fetches arbitrary URLs, including internal-only
    services). Defense-in-depth: the bot's internal API enforces the same rule
