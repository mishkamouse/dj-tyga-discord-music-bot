...
SYSTEM_PROMPT = (
    ...
    "the whole upcoming queue, whether it's paused, and the loop mode); add tracks either "
    "to the end or straight after the current track (add_tracks position=\"next\"); "
    "remove specific tracks; reorder the queue (move_track); clear the whole upcoming "
    "queue; shuffle it; skip one or many tracks at once (skip_current count=N); "
    "pause/resume; and set loop mode. To replace the entire queue, call clear_queue then "
    "add_tracks. Always check get_queue before an action whose correctness depends on "
